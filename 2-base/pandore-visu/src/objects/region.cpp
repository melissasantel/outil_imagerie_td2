...
Errc Reg1d::SaveData( FILE *df ) const {
   if (Labels()<= MAXUCHAR){
	 ...
	 if (Fencode((void*)&x,sizeof(x),1,df)<1)
      ...
      if (Fencode((void*)Vector(),sizeof(Reg1d::ValueType),(int)(ncol),df)<(size_t)(ncol))
	 return FAILURE;
   }
   return SUCCESS;
}
...
Errc Reg1d::LoadData( FILE *df ) {
   if (Labels()<= MAXUCHAR){
	 ...
	 *p=(Reg1d::ValueType)x;
      ...
      if (Fdecode((void*)Vector(),sizeof(Reg1d::ValueType),(int)(ncol),df)<(size_t)(ncol))
	 return FAILURE;
   }
   return SUCCESS;
}
 ...
 */
Reg1d &Reg1d::operator=( const Reg1d::ValueType val ) {
...
Errc Reg2d::LoadData( FILE *df ) {
   if (Labels()<= MAXUCHAR){
	 ...
	 *p=(Reg2d::ValueType)x;
      ...
      if (Fdecode((void*)Vector(),sizeof(Reg2d::ValueType),(int)(nrow*ncol),df)<(size_t)(nrow*ncol))
	 return FAILURE;
   }
   return SUCCESS;
}
 ...
 * This function copies val on each pixel of the `this' data.
 */
Reg2d &Reg2d::operator=( const Reg2d::ValueType val ) {
...
Reg2d &Reg2d::operator=( const Reg2d &ims ) {
   nlabels = ims.nlabels;
   memcpy(Vector(),ims.Vector(),nrow*ncol*sizeof(Reg2d::ValueType));
   return *this;
}
 ...
 * This function copies val on each pixel of the `this' data.
 */
Reg3d &Reg3d::operator=( const Reg3d::ValueType val ) {
